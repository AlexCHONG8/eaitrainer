...
function openVideoModal(videoSrc, title) {
        ...
        if (!hasShownError) {
            document.getElementById('videoCaption').innerHTML = `
                <div style="color: #4a90e2; font-size: 16px;">
                    🔄 视频加载中...<br>
                    <span style="font-size: 14px; opacity: 0.8;">请稍候</span>
                </div>
            `;
        }
    };
    ...
    loadingTimeout = setTimeout(() => {
        if (video.readyState === 0) { // 如果10秒后仍未开始加载
            hasShownError = true;
            console.error('视频加载超时:', videoSrc);
            document.getElementById('videoCaption').innerHTML = `
                <div style="color: #ff6b6b; font-size: 16px;">
                    ⚠️ 视频加载超时<br>
                    <span style="font-size: 14px; opacity: 0.8;">请检查网络连接或文件路径: ${videoSrc}</span>
                </div>
            `;
            ...
            errorDiv.innerHTML = `
                ...
                <button onclick="location.reload()" style="margin-top: 15px; padding: 8px 16px; background: #4a90e2; color: white; border: none; border-radius: 4px; cursor: pointer;">重新加载页面</button>
            `;
            video.parentNode.insertBefore(errorDiv, video.nextSibling);
        }
    }, 10000); // 10秒超时
    
    // 显示初始加载提示（延迟1秒显示，避免闪烁）
    ...
    video.onloadeddata = function() {
        ...
        // 移除可能存在的错误提示
        const errorDiv = video.parentNode.querySelector('#videoErrorDiv');
        if (errorDiv) {
            errorDiv.remove();
        }
    };
    ...
    video.onerror = function() {
        ...
        // 延迟处理错误，给视频更多加载时间
        setTimeout(() => {
            if (video.readyState === 0 && !hasShownError) {
                hasShownError = true;
                console.error('视频加载失败:', videoSrc);
                document.getElementById('videoCaption').innerHTML = `
                    <div style="color: #ff6b6b; font-size: 16px;">
                        ⚠️ 视频文件错误<br>
                        <span style="font-size: 14px; opacity: 0.8;">文件可能不存在或格式不支持</span>
                    </div>
                `;
...
function closeVideoModal() {
    ...
    if (videoModal) {
        ...
        // 清理错误提示和播放提示
        const errorDivs = videoModal.querySelectorAll('#videoErrorDiv, div[style*="pointer-events: none"]');
